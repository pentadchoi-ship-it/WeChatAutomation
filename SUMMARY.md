...
- 增加了 Windows 微信朋友圈探针链路，已验证图片查看器批量保存、正文右键复制、折叠长文完整复制、转发链接 URL 保存策略、`profile.json` / `moments/{id}/moment.json` 两层结构和 UIA/MSAA 可行性探测。
...
- 转发的视频号、文章、网页、小程序等外部引用只保存 URL，不下载转发内容里的媒体。
